"""
maya_live_sync.py
Maya 側スクリプト(SP Live Sync, B案:自前パイプライン)
...
# 2026.07.26: ブロック終端の検出に使う専用マーカー。以前は汎用的な
# Pythonパターン("except Exception:\n    pass\n")を終端として検索して
# いたため、ユーザーがuserSetup.pyへ独自に追記したコードの中に偶然同じ
# パターンが現れると、削除・書き換え範囲を誤検出する恐れがあった。
# 固有の文字列にすることでその曖昧さを無くす。
        ...
        if os.path.isfile(path):
            ...
            if REGISTER_MARKER in existing:
                ...
                if end_idx != -1:
                    ...
                    # 後方互換: REGISTER_END_MARKER が導入される前に登録された
                    # 旧形式のブロックには専用の終端マーカーが無いため、
                    # 従来の終端パターンでフォールバック検出する。一度この
                    # 関数で書き直されれば、以後は新しい終端マーカー付きの
                    # ブロックになる。
                    legacy_end_marker = "except Exception:\n    pass\n"
                    end_idx = existing.find(legacy_end_marker, start)
                    if end_idx == -1:
